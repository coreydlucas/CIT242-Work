//You Do It HynosisCenter pg.151
...
int main()
{
	...
	cout << "First session is on the following day: " << aClient.firstSessionMonth << "/" << aClient.firstSessionDay << "/" << aClient.firstSessionYear << endl;

	for (count = 2, nextSessionMonth = aClient.firstSessionMonth, nextSessionYear = aClient.firstSessionYear; count <= aClient.numSessions; ++count)
	{
		nextSessionMonth += 1;
		...
		cout << "Session #" << count << " is on: " << nextSessionMonth << "/" << aClient.firstSessionDay << "/" << nextSessionYear << endl;
